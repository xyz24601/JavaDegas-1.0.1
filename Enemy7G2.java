...
public class Enemy7G2
{
  ...
  public Enemy7G2()
  {
    for (int i = 0; i < MAXENUM; i++)
      enemy[i] = new Enemy7();
    alive = false;              /* needed for use by enemy7w2 */
  }
...
//  public boolean init(double ix, boolean pwr, int imaxenum, double speed)
  public boolean init(double ix, boolean pwr, int imaxenum, double speed,
                      int rdelay)
  ...
  \*    Return: boolean - true if init, false otherwise */
  {
    ...
    else
    {
      ...
      for (int i = 0; i < maxenum; i++)
      {                         /* vertically evenly place them */
        enemy[i].reset(ix, (i + 1) * JavaDegas.G_HEIGHT / (maxenum + 1),
                       speed, pwr, rdelay);
        dead[i] = false;
      }
      return(true);
